        ...
        <Card key={index} className="overflow-hidden hover:shadow-lg transition-shadow duration-300">
          <CardHeader className="pb-2">
            <div className="flex justify-between items-center">
              ...
              {index < 3 && (
                <Trophy
                  className={`h-6 w-6 ${index === 0 ? "text-yellow-500" : index === 1 ? "text-gray-400" : "text-amber-700"
                    }`}
                />
              )}
            </div>
              ...
              <div className="w-full md:w-1/2">
                ...
                <div className="mt-3">
                  ...
                  <div className="space-y-1">
                    {student.details?.map((detail, index) => <div key={detail.component + index + "row"} className="flex items-center text-sm">
                      <Star className="h-3 w-3 text-[#7635E9] mr-2" />
                      <span><span className="capitalize">{detail.component}</span>: {detail.score}</span>
                    </div>)}
                  </div>
                </div>
              </div>
